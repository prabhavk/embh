...
public:
	int degree = 0;
	int pattern_index = -1;  // -1 means not a leaf/observed vertex
	int timesVisited = 0;
	bool observed = 0;
...
class clique {
	...
	// Reference to packed patterns for direct access (avoids DNAcompressed)
	...
	clique (SEM_vertex * x, SEM_vertex * y, PackedPatternStorage* patterns = nullptr) {
		this->x = x;
		this->y = y;
		this->name = to_string(x->id) + "-" + to_string(y->id);
		this->logScalingFactorForClique = 0;
		this->isLeafClique = false;
		this->packed_patterns = patterns;
	}
...
void clique::SetInitialPotentialAndBelief(int site) {
	...
	if (y->observed) {
		...
		int dna_y = 4; // default to gap
		if (this->packed_patterns != nullptr && y->pattern_index >= 0) {
			dna_y = this->packed_patterns->get_base(site, y->pattern_index);
		}
		if (dna_y == 4) {
			// cout << "case 1" << endl;
			matchingCase = 1; // gap
...
void clique::ApplyEvidenceForSite(int site) {
	...
	if (y->observed && y->pattern_index >= 0 && this->packed_patterns != nullptr) {
		int dna_y = this->packed_patterns->get_base(site, y->pattern_index);
...
vector<int> clique::GetSubtreeSignature(int site) {
	...
	for (SEM_vertex* leaf : this->subtreeLeaves) {
		if (this->packed_patterns != nullptr && leaf->pattern_index >= 0) {
			signature.push_back(this->packed_patterns->get_base(site, leaf->pattern_index));
		} else {
			signature.push_back(4); // gap if no pattern data
		}
	}
	return signature;
}
...
public:
	...
	// Reference to packed patterns for direct access
	PackedPatternStorage* packed_patterns = nullptr;
...
public:	
	...
	void SetEdgesForTreeTraversalOperations();
	void SetLeaves();
	void SetLeavesFromPatternIndices();
	void PopulateDNACompressedFromPatterns();
...
void SEM::ConstructCliqueTree() {
	...
	// Pass packed_patterns reference to cliqueTree
	this->cliqueT->packed_patterns = this->packed_patterns;
	SEM_vertex * root;
	for (clique * C : this->cliqueT->cliques) {
		delete C;
	}
	...
	for (pair <SEM_vertex *, SEM_vertex *> edge : this->edgesForPreOrderTreeTraversal) {
		// Pass packed_patterns to each clique for direct pattern access
		clique * C = new clique(edge.first, edge.second, this->packed_patterns);
...
void SEM::ComputeLogLikelihood() {
	...
	for (int site = 0; site < this->num_dna_patterns; site++) { // parallelize computation here 
		...
		for (auto& edge : this->edgesForPostOrderTreeTraversal) {
			...
			if (c->outDegree == 0) {
				int c_base = 4; // default gap
				if (this->packed_patterns != nullptr && c->pattern_index >= 0) {
					c_base = this->packed_patterns->get_base(site, c->pattern_index);
				}
				if (c_base != 4) { // != 4
					conditionalLikelihood.fill(0.0);
					conditionalLikelihood[c_base] = 1.0;
				} else {
			...
			if (conditionalLikelihoodMap.find(p) == conditionalLikelihoodMap.end()) {
				if (p->id > this->numberOfObservedVertices - 1) {
					...
					int base = 4; // default gap
					if (this->packed_patterns != nullptr && p->pattern_index >= 0) {
						base = this->packed_patterns->get_base(site, p->pattern_index);
					}
					if (base < 4) {
						// Valid DNA base (0-3)
						conditionalLikelihood.fill(0.0);
...
void SEM::embh_aitken(int max_iterations) {
    ...
    cout << "  Root probs:  [" << fixed << setprecision(8)
         << this->rootProbability[0] << ", " << this->rootProbability[1]
         << ", " << this->rootProbability[2] << ", " << this->rootProbability[3] << "]" << endl;
    cout << "====================================================" << endl;
}
...
array <double, 4> SEM::GetObservedCountsForVariable(SEM_vertex * v) {
	...
	// Use packed patterns directly instead of DNAcompressed
	if (this->packed_patterns != nullptr && v->pattern_index >= 0) {
		for (int site = 0; site < this->num_dna_patterns; site ++) {
			int base = this->packed_patterns->get_base(site, v->pattern_index);
			if (base < 4) { // FIX_AMB
				observedCounts[base] += this->DNAPatternWeights[site];
			}
		}
	}
...
void SEM::SetLeavesFromPatternIndices() {
	// Mark vertices as observed if they have a valid pattern_index (>= 0)
	// This is called after ReadPatternsFromFile which sets pattern_index
	this->leaves.clear();
	this->numberOfObservedVertices = 0;
	this->numberOfInputSequences = 0;

	for (pair<int, SEM_vertex*> idPtrPair : * this->vertexMap){
		SEM_vertex* v = idPtrPair.second;
		if (v->pattern_index >= 0) {
			// This vertex has a pattern index, so it's a leaf/observed vertex
			v->observed = 1;
	...
	cout << "Marked " << this->numberOfObservedVertices << " vertices as observed (leaves)" << endl;
}

void SEM::PopulateDNACompressedFromPatterns() {
	// Populate DNAcompressed for all observed vertices from packed patterns
	// This is needed for EM algorithm which uses DNAcompressed to access site data
	if (packed_patterns == nullptr) {
		cerr << "Error: No patterns loaded. Call ReadPatternsFromFile() first." << endl;
		return;
	}
	...
	// Resize DNAcompressed for all observed vertices
	for (auto& pair : *this->vertexMap) {
		SEM_vertex* v = pair.second;
		if (v->observed) {
			v->DNAcompressed.resize(num_patterns_from_file, 4);  // 4 = gap
		}
	}

	// Populate from packed patterns
	for (int pattern_idx = 0; pattern_idx < num_patterns_from_file; pattern_idx++) {
		vector<uint8_t> pattern = packed_patterns->get_pattern(pattern_idx);
	...
	cout << "Populated DNAcompressed for " << this->numberOfObservedVertices << " observed vertices with " << num_patterns_from_file << " patterns" << endl;
}
...
manager::manager(const string edge_list_file_name,
		...
		// Set up tree structure from edge list (creates all vertices)
		this->P->SetEdgesFromTopologyFile(edge_list_file_name);
		this->P->SetVertexVector();

		// Read patterns and set up pattern indices for vertices
		cout << "\n=== Loading patterns from file ===" << endl;
		this->P->ReadPatternsFromFile(pattern_file_name, taxon_order_file_name);

		// Mark leaf vertices as observed (those with pattern_index >= 0)
		this->P->SetLeavesFromPatternIndices();

		// No longer need to populate DNAcompressed - using packed patterns directly
