...
// if one is only interested in computing log-likelihood 
// then it suffices to send messages from leaves to root
// and compute the belief at the root
// use the belief along with log scaling factors to compute log-likelihood
...
public:	
	...
	void ComputeLogLikelihoodUsingCompressedSequencesWithPropagationAlgorithm();
...
/*
	First confirm log-likelihood score, then optimize further
	split function into two parts
	set initial potential as transition matrix (common across sites)
	multiply in the evidence for initial potential (only for cliques containing observed variables, and is different for each site)
*/		
void SEM::ComputeLogLikelihoodUsingCompressedSequencesWithPropagationAlgorithm() {
	this->logLikelihood = 0;
	clique * rootClique = this->cliqueT->root;
	// parallelize here if needed
	for (int site = 0; site < this->num_dna_patterns; site++) {		
		this->cliqueT->SetSite(site);		
		this->cliqueT->InitializePotentialAndBeliefs();		
		this->cliqueT->CalibrateTree(); // site log likelihood can computed after this step
		
		// CRITICAL FIX: Although root probabilities are incorporated in the initial potential
		// (line 446 in SetInitialPotentialAndBelief), they get normalized away during the
		// belief propagation in ComputeBelief(). The final belief is a normalized conditional
		// distribution. We must re-apply root probabilities, just like the pruning algorithm does.
...
EMBH::EMBH(const string edge_list_file_name,
		...
		cout << "log-likelihood using pruning algorithm and compressed sequences is " << setprecision(20) << this->P->logLikelihood << endl;		
		...
		this->P->ConstructCliqueTree();
		this->P->ComputeLogLikelihoodUsingCompressedSequencesWithPropagationAlgorithm();
		cout << "log-likelihood using propagation algorithm and compressed sequences is " 
		     << setprecision(20) << this->P->logLikelihood << endl;
