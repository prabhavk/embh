...
public:
	...
	vector <SEM_vertex_old *> neighbors;
	vector <SEM_vertex_old *> children;
	array <double, 4> root_prob_hss;
	SEM_vertex_old * parent = this;
	void AddNeighbor(SEM_vertex_old * v_ptr);
	void RemoveNeighbor(SEM_vertex_old * v_ptr);
	void AddParent(SEM_vertex_old * v_ptr);
	void RemoveParent();
	void AddChild(SEM_vertex_old * v_ptr);
	void RemoveChild(SEM_vertex_old * v_ptr);
	...
	SEM_vertex_old (int idToAdd, vector <int> compressedSequenceToAdd) {
		this->id = idToAdd;
...
void SEM_vertex_old::SetVertexLogLikelihood(double vertexLogLikelihoodToSet) {
	this->vertexLogLikelihood = vertexLogLikelihoodToSet;
}

void SEM_vertex_old::AddParent(SEM_vertex_old * v) {
	this->parent = v;
	this->inDegree += 1;
}

void SEM_vertex_old::RemoveParent() {
	this->parent = this;
	this->inDegree -=1;
}

void SEM_vertex_old::AddChild(SEM_vertex_old * v) {
	this->children.push_back(v);
	this->outDegree += 1;
}

void SEM_vertex_old::RemoveChild(SEM_vertex_old * v) {
	int ind = find(this->children.begin(),this->children.end(),v) - this->children.begin();
	this->children.erase(this->children.begin()+ind);
	this->outDegree -=1;
}

void SEM_vertex_old::AddNeighbor(SEM_vertex_old * v) {
	this->degree += 1;
	this->neighbors.push_back(v);
}

void SEM_vertex_old::RemoveNeighbor(SEM_vertex_old * v) {
	this->degree -= 1;
...
class clique {
	...
	SEM_vertex_old * x;
	SEM_vertex_old * y;
	std::array <double, 4> MarginalizeOverVariable(SEM_vertex_old * v);
//	emtr::Md DivideBeliefByMessageMarginalizedOverVariable(SEM_vertex * v);	
	...
	clique (SEM_vertex_old * x, SEM_vertex_old * y) {
		this->x = x;
		this->y = y;
...
std::array <double, 4> clique::MarginalizeOverVariable(SEM_vertex_old * v) {
...
public:
	...
	map < pair <SEM_vertex_old *, SEM_vertex_old *>, emtr::Md> marginalizedProbabilitiesForVariablePair;
	map < pair <clique *, clique *>, pair <SEM_vertex_old *, SEM_vertex_old *>> cliquePairToVariablePair;
	int site;
	clique * root;
	bool rootSet;
	...
	emtr::Md GetP_XZ(SEM_vertex_old * X, SEM_vertex_old * Y, SEM_vertex_old * Z);	
	SEM_vertex_old * GetCommonVariable(clique * Ci, clique * Cj);
	tuple <SEM_vertex_old *,SEM_vertex_old *,SEM_vertex_old *> GetXYZ(clique * Ci, clique * Cj);
	cliqueTree () {
		rootSet = 0;
	}
	~cliqueTree () {
...
tuple <SEM_vertex_old *,SEM_vertex_old *,SEM_vertex_old *> cliqueTree::GetXYZ(clique * Ci, clique * Cj) {
	SEM_vertex_old * X; SEM_vertex_old * Y; SEM_vertex_old * Z;
	SEM_vertex_old * Y_temp;
	clique * Cl;
	...
	return (tuple <SEM_vertex_old *,SEM_vertex_old *,SEM_vertex_old *>(X,Y,Z));
}

emtr::Md cliqueTree::GetP_XZ(SEM_vertex_old * X, SEM_vertex_old * Y, SEM_vertex_old * Z) {
	emtr::Md P_XY; emtr::Md P_YZ;
	emtr::Md P_ZGivenY; emtr::Md P_XZ;
	
    if (X->id < Y->id) {
		P_XY = this->marginalizedProbabilitiesForVariablePair[pair<SEM_vertex_old *, SEM_vertex_old *>(X,Y)];
	} else {		
		P_XY = emtr::MT(this->marginalizedProbabilitiesForVariablePair[pair<SEM_vertex_old *, SEM_vertex_old *>(Y,X)]);
	}
	if (Y->id < Z->id) {
		P_YZ = this->marginalizedProbabilitiesForVariablePair[pair<SEM_vertex_old *, SEM_vertex_old *>(Y,Z)];
	} else {		
		P_YZ = emtr::MT(this->marginalizedProbabilitiesForVariablePair[pair<SEM_vertex_old *, SEM_vertex_old *>(Z,Y)]);
	}
...
SEM_vertex_old * cliqueTree::GetCommonVariable(clique * Ci, clique * Cj) {
	SEM_vertex_old * commonVariable;
...
void cliqueTree::SetMarginalProbabilitesForEachEdgeAsBelief() {
	...
	for (clique * C: this->cliques) {
		if (C->x->id < C->y->id) {			
			this->marginalizedProbabilitiesForVariablePair.insert(pair<pair<SEM_vertex_old *, SEM_vertex_old *>, emtr::Md>(pair<SEM_vertex_old *, SEM_vertex_old *>(C->x,C->y),C->belief));
		} else {			
			this->marginalizedProbabilitiesForVariablePair.insert(pair<pair<SEM_vertex_old *, SEM_vertex_old *>, emtr::Md>(pair<SEM_vertex_old *, SEM_vertex_old *>(C->y,C->x),emtr::MT(C->belief)));
		}
	}
}
...
void cliqueTree::ComputeMarginalProbabilitesForEachVariablePair() {	
	...
	SEM_vertex_old * X; SEM_vertex_old * Z;
	SEM_vertex_old * Y;
	...
	for (pair <clique *, clique *> cliquePair : this->cliquePairsSortedWrtLengthOfShortestPath) {
		...
		this->cliquePairToVariablePair.insert(pair <pair <clique *, clique *>,pair <SEM_vertex_old *, SEM_vertex_old *>>(pair <clique *, clique *>(Ci,Cj), pair <SEM_vertex_old *, SEM_vertex_old *>(X,Z)));
		P_XZ = this->GetP_XZ(X, Y, Z);
		if (X->id < Z->id) {
			this->marginalizedProbabilitiesForVariablePair.insert(pair<pair<SEM_vertex_old *, SEM_vertex_old *>, emtr::Md>(pair<SEM_vertex_old *, SEM_vertex_old *>(X,Z),P_XZ));			
		} else {			
			this->marginalizedProbabilitiesForVariablePair.insert(pair<pair<SEM_vertex_old *, SEM_vertex_old *>, emtr::Md>(pair<SEM_vertex_old *, SEM_vertex_old *>(Z,X),emtr::MT(P_XZ)));
		}
	}
}
...
public:
	...
	map <string,int> mapDNAtoInteger;
	map <int, SEM_vertex_old*> * vertexMap;
	vector <SEM_vertex_old*> vertices;
	map <pair<SEM_vertex_old *,SEM_vertex_old *>,emtr::Md> * M_hss;
	...
	// ofstream * logFile;
	SEM_vertex_old * root;
	vector < pair <SEM_vertex_old *, SEM_vertex_old *>> edgesForPostOrderTreeTraversal;
	vector < pair <SEM_vertex_old *, SEM_vertex_old *>> edgesForPreOrderTreeTraversal;	
	vector < pair <SEM_vertex_old *, SEM_vertex_old *>> edgesForChowLiuTree;
	vector < pair <SEM_vertex_old *, SEM_vertex_old *>> directedEdgeList;
	map <pair <SEM_vertex_old *, SEM_vertex_old *>, double> edgeLengths;
	vector < SEM_vertex_old *> leaves;
	vector < SEM_vertex_old *> preOrderVerticesWithoutLeaves;
	vector < SEM_vertex_old *> postOrderVertices;
	map < pair <SEM_vertex_old * , SEM_vertex_old *>, emtr::Md > expectedCountsForVertexPair;
	map < pair <SEM_vertex_old * , SEM_vertex_old *>, emtr::Md > posteriorProbabilityForVertexPair;
	map < SEM_vertex_old *, array <double,4>> expectedCountsForVertex; 
	map < SEM_vertex_old *, array <double,4>> posteriorProbabilityForVertex;
	...
	map <pair<SEM_vertex_old *,SEM_vertex_old *>,double> edgeLogLikelihoodsMap;
	SEM_vertex_old * externalVertex;
	void AddArc(SEM_vertex_old * from, SEM_vertex_old * to);
	void RemoveArc(SEM_vertex_old * from, SEM_vertex_old * to);
	...
	void AddExpectedCountMatrices(map < pair <SEM_vertex_old * , SEM_vertex_old *>, emtr::Md > expectedCountsForVertexPair);
...
//	void AddCompressedSequencesAndNames(map<string,vector<int>> sequencesList, vector <vector <int>> sitePatternRepeats);	
	...
	void EMTrifle_started_with_parsimony_rooted_at(SEM_vertex_old *v, int layer);
	void EMTrifle_started_with_dirichlet_rooted_at(SEM_vertex_old *v, int layer);
	void EMTrifle_started_with_HSS_rooted_at(SEM_vertex_old *v, int layer);
	tuple<int,double,double,double,double> EM_started_with_parsimony_rooted_at(SEM_vertex_old *v);
	tuple<int,double,double,double,double> EM_started_with_dirichlet_rooted_at(SEM_vertex_old *v);	
	tuple<int,double,double,double,double> EM_started_with_HSS_parameters_rooted_at(SEM_vertex_old *v);
	...
	void RootTreeAlongEdge(SEM_vertex_old * u, SEM_vertex_old * v);
	void SelectEdgeIncidentToVertexViaMLForBH(SEM_vertex_old * v);
	...
	void RemoveEdgeLength(SEM_vertex_old * u, SEM_vertex_old * v);
	void AddEdgeLength(SEM_vertex_old * u, SEM_vertex_old * v, double t);
	double GetEdgeLength(SEM_vertex_old * u, SEM_vertex_old * v);
	double ComputeEdgeLength(SEM_vertex_old * u, SEM_vertex_old * v);
	void SetEdgeLength(SEM_vertex_old * u, SEM_vertex_old * v, double t);
	...
	void AddSubforestOfInterest(SEM_old * localPhylogeneticTree);
	...
	SEM_vertex_old * GetVertex(string v_name);
	bool ContainsVertex(string v_name);
	int GetEdgeIndex (int vertexIndex1, int vertexIndex2, int numberOfVertices);
	emtr::Md GetP_yGivenx(emtr::Md P_xy);
	emtr::Md ComputeTransitionMatrixUsingAncestralStates(SEM_vertex_old * p, SEM_vertex_old * c);
	emtr::Md ComputeTransitionMatrixUsingAncestralStatesForTrifle(SEM_vertex_old * p, SEM_vertex_old * c, int layer);
	array <double, 4> GetBaseComposition(SEM_vertex_old * v);
	array <double, 4> GetBaseCompositionForTrifle(SEM_vertex_old * v, int layer);
	array <double, 4> GetObservedCountsForVariable(SEM_vertex_old * v);
	...
	void RootTreeAtVertex(SEM_vertex_old * r);
	...
	double GetMLDistance_NR_Reversible_Tuples(SEM_vertex_old* u, SEM_vertex_old* v, const Eigen::Matrix<double,20,20>& Q, const Eigen::VectorXd& pi, int max_iter = 50, double grad_tol = 1e-8, double ll_tol = 1e-10);
	void ComputeLogLikelihoodUsingExpectedDataCompletion();
	pair <bool, SEM_vertex_old *> CheckAndRetrieveSingletonHiddenVertex();
	pair <bool, SEM_vertex_old *> CheckAndRetrieveHiddenVertexWithOutDegreeZeroAndInDegreeOne();
	pair <bool, SEM_vertex_old *> CheckAndRetrieveHiddenVertexWithOutDegreeOneAndInDegreeOne();
	pair <bool, SEM_vertex_old *> CheckAndRetrieveHiddenVertexWithOutDegreeOneAndInDegreeZero();
	pair <bool, SEM_vertex_old *> CheckAndRetrieveHiddenVertexWithOutDegreeGreaterThanTwo();
	pair <bool, SEM_vertex_old *> CheckAndRetrieveObservedVertexThatIsTheRoot();
	pair <bool, SEM_vertex_old *> CheckAndRetrieveObservedVertexThatIsNotALeafAndIsNotTheRoot();
	double GetExpectedMutualInformation(SEM_vertex_old * u, SEM_vertex_old * v);
	...
	emtr::Md GetObservedCounts(SEM_vertex_old * u, SEM_vertex_old * v);
	void AddToExpectedCounts();
	void AddToExpectedCountsForEachVariable();
	void AddToExpectedCountsForEachVariablePair();
	emtr::Md GetExpectedCountsForVariablePair(SEM_vertex_old * u, SEM_vertex_old * v);
	emtr::Md GetPosteriorProbabilityForVariablePair(SEM_vertex_old * u, SEM_vertex_old * v);
	...
	void ComputeVertexLogLikelihood(SEM_vertex_old * v);
	void ComputeEdgeLogLikelihood(SEM_vertex_old * u, SEM_vertex_old * v);
	...
	void CompressDNASequencesSep20();
	...
	void ReadFasta();
	Eigen::Matrix <double,20,20> GetCountMatrix(SEM_vertex_old * u, SEM_vertex_old * v);
	vector <double> GetCountVector (SEM_vertex_old * u);
	double DyadicLogLikelihood(SEM_vertex_old * u, SEM_vertex_old * v, double t);
	double DyadicLogLikelihoodFirstDer(SEM_vertex_old * u, SEM_vertex_old * v, double t);
	// double DyadicLogLikelihoodSecondDer(SEM_vertex * u, SEM_vertex * v, double t);
	double GetNewtonRaphsonDistance(SEM_vertex_old * u, SEM_vertex_old * v);
	double GetJukesCantorDistance(SEM_vertex_old * u, SEM_vertex_old * v);
	...
	map <pair<SEM_vertex_old*, SEM_vertex_old*>,double> ML_distances;
	...
	SEM_old (double loglikelihood_conv_thresh, int max_EM_iter, bool verbose_flag_to_set) {
		...
		this->node_ind = 0;
		this->vertexMap = new map <int, SEM_vertex_old *> ;
		this->M_hss = new map <pair<SEM_vertex_old*,SEM_vertex_old*>,emtr::Md>;
	...
	~SEM_old () {
		for (pair <int, SEM_vertex_old * > idPtrPair : * this->vertexMap) {
...
int SEM_old::unique_non_gap_count_in_pattern(const std::vector<int>& pat) {    
...
void SEM_old::addToZeroEntriesOfRootProbability(array <double,4> & baseComposition) {
	double sum = 0.0;
...
void SEM_old::addToZeroEntriesOfTransitionMatrix(array<array<double,4>,4>& P) {
...
int SEM_old::first_index_gt(const std::vector<double>& cum, double thr) {
...
void SEM_old::SetParameters(string parameters_json) {
...
void SEM_old::SetAASiteCumWeight() {
...
Matrix20d SEM_old::ExpQt_fromSpectrum(double t) const {
    if (!spectrum.ok) {    
        return (t * Q_D).exp();
    }
...
void SEM_old::PrecomputeSpectrum() {
...
void SEM_old::AssignLSBranchLengthsToTree() {
    if (this->DM_LS.empty()) {
...
void SEM_old::SetFJTree(const fj::Graph& tree) {
    this->T = tree;
    this->T.rebuild();
}


void SEM_old::ComputeNRDistanceMatrix() {
    // --- settings ---
    ...
    for (int i = 0; i < n; ++i) {
        SEM_vertex_old* u = this->vertices[i];
        const std::string& ui = u->name;

        for (int j = i + 1; j < n; ++j) {
            SEM_vertex_old* v = this->vertices[j];
...
// using line search
void SEM_old::ComputeLSDistanceMatrix() {
    // --- settings ---
...
void SEM_old::BuildSIndex(std::vector<std::pair<int,int>>& idxS, int i0, int j0) const {
	idxS.clear();
    for (int i = 0; i < 20; ++i)
...
void SEM_old::PackTheta(const std::vector<std::pair<int,int>>& idxS, int p_ref,
...
void SEM_old::UnpackTheta(const std::vector<double>& theta,
...
double SEM_old::NMObjectiveNegLL(const std::vector<double>& theta,
                             const std::vector<std::pair<int,int>>& idxS,
                             int i0, int j0, int p_ref) {
    // Save
...
void SEM_old::OptimizeRateMatrixNelderMead(int max_iter, double tol, double step, int i0, int j0, int p_ref) {
    // Free S indices
...
double SEM_old::GetMLDistance_NR_Reversible_Tuples(SEM_vertex_old* u, SEM_vertex_old* v, const Eigen::Matrix<double,20,20>& Q, const Eigen::VectorXd& pi, int max_iter, double grad_tol, double ll_tol) {
    Eigen::Matrix<double,20,20> N = GetCountMatrix(u, v);
...
tuple<double,double,double> SEM_old::LL_Grad_Hess_Counts_Reversible_Tuple(const Eigen::Matrix<double,20,20>& N, const Eigen::VectorXd& pis, const Eigen::VectorXd& invpis, const Eigen::MatrixXd& U, const Eigen::VectorXd& lambdas, double t) const {
...
tuple<Eigen::Matrix<double,20,20>, Eigen::Matrix<double,20,20>, Eigen::Matrix<double,20,20>> SEM_old::EvalP_QP_Q2P_Tuple(const Eigen::VectorXd& pis, const Eigen::VectorXd& invpis, const Eigen::MatrixXd& U, const Eigen::VectorXd& lambdas, double t) const {
    Eigen::Matrix<double,20,20> UdexpUt   = Eigen::Matrix<double,20,20>::Zero();
...
tuple<Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd, Eigen::MatrixXd, Eigen::VectorXd> SEM_old::ReversibleSpecTuple(const Eigen::Matrix<double,20,20>& Q, const Eigen::VectorXd& pi_in) const {
...
double SEM_old::DyadicLogLikelihood(SEM_vertex_old * p, SEM_vertex_old * c, double t) {
	double ll = 0.0;	
...
Eigen::Matrix <double,20,20> SEM_old::GetCountMatrix(SEM_vertex_old * p, SEM_vertex_old * c) {
...
vector <double> SEM_old::GetCountVector(SEM_vertex_old * p) {
...
double SEM_old::GetJukesCantorDistance(SEM_vertex_old * u, SEM_vertex_old * v) {
	double t, p;
	p = 0;
	Eigen::Matrix <double,20,20> N = GetCountMatrix(u,v);
...
double SEM_old::DyadicLogLikelihoodFirstDer(SEM_vertex_old * p, SEM_vertex_old * c, double t) {
...
double SEM_old::GetNewtonRaphsonDistance(SEM_vertex_old * u, SEM_vertex_old * v) {		
	float p = 0;
	double t;
	int aa_u; int aa_v;
...
void SEM_old::ComputeMLDistancesForAAsequences() {
	double ML_d;
	for (SEM_vertex_old * u: this->vertices) {
		for (SEM_vertex_old * v: this->vertices) {
			if (u < v) {
...
void SEM_old::SetDayhoffExchangeabilityAndRootProbabilityParameters() {
...
void SEM_old::SetRateMatrixParameters() {
	int K = 20;
...
array <double, 4> SEM_old::sample_pi() {
	return sample_dirichlet(this->alpha_pi, rng());
}

array <double, 4> SEM_old::sample_M_row() {
	return sample_dirichlet(this->alpha_M_row, rng());
}

array <double, 4> SEM_old::sample_dirichlet(const array<double, 4>& alpha, mt19937_64& gen) {
        array<double, 4> x{};
        double sum = 0.0;
        for (size_t i = 0; i < 4; ++i) {
...
void SEM_old::set_alpha_PI(double a1, double a2, double a3, double a4) {
	this->alpha_pi[0] = a1;
	this->alpha_pi[1] = a2;
	this->alpha_pi[2] = a3;
	this->alpha_pi[3] = a4;	
...
void SEM_old::set_alpha_M_row(double a1, double a2, double a3, double a4) {
...
void SEM_old::AddDuplicatedSequencesToRootedTree(MST * M) {
	...
	for (pair <string, vector <string> > uniq_seq_name_2_dupl_seq_name_vec : M->unique_seq_id_2_dupl_seq_ids) {
		...
		v_id += 1;
		h = new SEM_vertex_old(v_id,emptySequence);
		this->vertexMap->insert(pair<int,SEM_vertex_old*>(h->id,h));
		...
		for (string dupl_seq_name: dupl_seq_name_vec) {
			v_id += 1;
			d = new SEM_vertex_old(v_id,emptySequence);
			d->name = dupl_seq_name;
			this->vertexMap->insert(pair<int,SEM_vertex_old*>(d->id,d));
...
void SEM_old::AddDuplicatedSequencesToUnrootedTree(MST * M) {
	// Store dupl seq names in uniq seq vertex
	double t;
	string uniq_seq_name;
	vector <string> dupl_seq_name_vec;
...
void SEM_old::SetEdgeAndVertexLogLikelihoods() {
	SEM_vertex_old * u;	SEM_vertex_old * v;
	int u_id; int v_id;	
...
void SEM_old::ComputeVertexLogLikelihood(SEM_vertex_old * v) {
...
void SEM_old::ComputeEdgeLogLikelihood(SEM_vertex_old* x, SEM_vertex_old * y) {	
...
void SEM_old::SetWeightedEdgesToAddToGlobalPhylogeneticTree() {
...
void SEM_old::SetAncestralSequencesString() {
	vector <SEM_vertex_old *> verticesOfInterest;
	int u_id; int v_id;
	vector <int> fullSeq;
	string DNAString;
	SEM_vertex_old * u;	SEM_vertex_old * v;
...
void SEM_old::SetNeighborsBasedOnParentChildRelationships() {
	this->ClearUndirectedEdges();
	SEM_vertex_old * p; SEM_vertex_old * c;
	for (pair<SEM_vertex_old*, SEM_vertex_old*> edge : this->edgesForPreOrderTreeTraversal) {
		tie (p, c) = edge;
		p->AddNeighbor(c);
		c->AddNeighbor(p);
	}
...
void SEM_old::SetIdsForObservedVertices(vector <int> idsOfObservedVerticesToAdd) {
	this->idsOfObservedVertices = idsOfObservedVerticesToAdd;
}

void SEM_old::SetNumberOfInputSequences(int numOfInputSeqsToSet) {
	this->numberOfInputSequences = numOfInputSeqsToSet;	
}
void SEM_old::ComputeBIC() {
	...
	for (SEM_vertex_old * v : this->root->children) {
...
void SEM_old::SetModelSelectionCriterion(string modelSelectionCriterionToSet) {
	this->modelSelectionCriterion = modelSelectionCriterionToSet;
}

void SEM_old::SetFlagForFinalIterationOfSEM() {
	this->finalIterationOfSEM = 1;
}


void SEM_old::ResetData() {
	for (pair<int,SEM_vertex_old*> idPtrPair : *this->vertexMap) {
...
SEM_vertex_old * SEM_old::GetVertex(string v_name){
...
int SEM_old::GetVertexId(string v_name) {
	SEM_vertex_old * v;
	int idToReturn = -10;
	for (pair<int,SEM_vertex_old*> idPtrPair : *this->vertexMap) {
		v = idPtrPair.second;
...
void SEM_old::SwapRoot() {
	SEM_vertex_old * root_current;
	SEM_vertex_old * vertexNamedHRoot;
	vector <SEM_vertex_old *> childrenOfCurrentRoot;
	vector <SEM_vertex_old *> childrenOfVertexNamedHRoot;
...
//		this->ComputeLogLikelihood();		
		...
		for (SEM_vertex_old * c: childrenOfVertexNamedHRoot) {
			this->RemoveArc(vertexNamedHRoot,c);
			this->AddArc(root_current,c);
		}
		
		for (SEM_vertex_old * c: childrenOfCurrentRoot) {			
...
int SEM_old::GetEdgeIndex (int vertexIndex1, int vertexIndex2, int numberOfVertices) {
	int edgeIndex;
	edgeIndex = numberOfVertices*(numberOfVertices-1)/2;
	edgeIndex -= (numberOfVertices-vertexIndex1)*(numberOfVertices-vertexIndex1-1)/2;
	edgeIndex += vertexIndex2 - vertexIndex1 - 1;
...
char SEM_old::GetDNAfromIndex(int dna_index){
...
int SEM_old::ConvertAAtoIndex(char aa) {
...
int SEM_old::ConvertDNAtoIndex(char dna){
	int value;
	switch (dna)
	{
	case 'A':
...
void SEM_old::SetBHparameters() {
	this->ReadProbabilities();	
}

void SEM_old::ReparameterizeBH() {
	...
	SEM_vertex_old * p; SEM_vertex_old * c; array <double,4> pi_p; array <double,4> pi_c;
	M_hss->clear();
	for (pair<SEM_vertex_old*, SEM_vertex_old*> edge : this->edgesForPreOrderTreeTraversal) {
		p = edge.first;
		c = edge.second;
...
void SEM_old::ReadProbabilities() {
    ...
    while (std::getline(inputFile, line)) {
        ...
        switch (num_words) {
            ...
            case 16: {                
                SEM_vertex_old* n = this->GetVertex(node_name);
...
void SEM_old::WriteProbabilities(string fileName) {
...
void SEM_old::ReadRootedTree(string treeFileName) {
	string u_name;
	string v_name;
	int u_id;
	int v_id;
	...
	for (string name: leafNames) {
		SEM_vertex_old * v = new SEM_vertex_old(v_id, emptySequence);
		v->name = name;
		v->observed = 1;
		this->vertexMap->insert(pair<int,SEM_vertex_old*>(v_id,v));
		v_id += 1;
	}
	...
	this->root = new SEM_vertex_old (((2 * n) - 2), emptySequence);	
	this->root->name = rootName;
	this->vertexMap->insert(pair <int,SEM_vertex_old*> (((2 * n) - 2), this->root));
	edgeListFile.clear();
...
bool SEM_old::IsTreeInCanonicalForm() {
...
void SEM_old::AddAllSequences(string fileName) {
	vector <int> recodedSequence;
...
void SEM_old::ResetAncestralSequences() {
...
void SEM_old::RemoveEdgeLength(SEM_vertex_old * u, SEM_vertex_old * v) {
	pair <SEM_vertex_old *, SEM_vertex_old *> vertexPair;
	if (u->id < v->id) {
...
void SEM_old::AddEdgeLength(SEM_vertex_old * u, SEM_vertex_old * v, double t) {	
	pair <SEM_vertex_old *, SEM_vertex_old *> vertexPair;
	if (u->id < v->id) {
		vertexPair = make_pair(u,v);
	} else {
...
double SEM_old::GetEdgeLength(SEM_vertex_old * u, SEM_vertex_old * v) {
	double t;
	pair <SEM_vertex_old *, SEM_vertex_old *> vertexPair;
	if (u->id < v->id) {
		vertexPair = make_pair(u,v);
...
double SEM_old::ComputeEdgeLength(SEM_vertex_old * u, SEM_vertex_old * v) {
	double t = 0;
	int dna_u; int dna_v; 
...
void SEM_old::SetEdgeLength(SEM_vertex_old * u, SEM_vertex_old * v, double t) {
	pair <SEM_vertex_old *, SEM_vertex_old *> vertexPair;
	if (u->id < v->id) {
		vertexPair = make_pair(u,v);
	} else {
...
void SEM_old::StoreEdgeListAndSeqToAdd() {
	this->weightedEdgeListString = "";		
	this->sequencesToAddToGlobalPhylogeneticTree.clear();
	this->weightedEdgesToAddToGlobalPhylogeneticTree.clear();
	SEM_vertex_old * u; SEM_vertex_old * v;	
	double t;	
	for (pair <SEM_vertex_old *, SEM_vertex_old *> vertexPair : this->edgesForPostOrderTreeTraversal) {
		tie (u, v) = vertexPair;		
		if (u->parent != u) {
...
emtr::Md SEM_old::ComputeTransitionMatrixUsingAncestralStatesForTrifle(SEM_vertex_old * p, SEM_vertex_old * c, int layer) {	
	emtr::Md P = emtr::Md{};
	// for (int i = 0; i < 4; i ++) {		
	// 	P[i][i] = 1.0;
	// }			
...
emtr::Md SEM_old::ComputeTransitionMatrixUsingAncestralStates(SEM_vertex_old * p, SEM_vertex_old * c) {	
	emtr::Md P = emtr::Md{};			
	int dna_p; int dna_c;
...
void SEM_old::SetPrefixForOutputFiles(string prefix_for_output_files_to_set){
	this->prefix_for_output_files = prefix_for_output_files_to_set;
}

void SEM_old::WriteRootedTreeInNewickFormat(string newickFileName) {
	vector <SEM_vertex_old *> verticesToVisit;
	SEM_vertex_old * c;
	SEM_vertex_old * p;	
	double edgeLength;	
	for (pair <int, SEM_vertex_old *> idAndVertex : * this->vertexMap) {
...
void SEM_old::WriteCliqueTreeToFile(string cliqueTreeFileName) {
...
void SEM_old::WriteUnrootedTreeAsEdgeList(string fileName) {
	ofstream treeFile;
	...
	for (pair <int, SEM_vertex_old *> idPtrPair : * this->vertexMap) {
		v = idPtrPair.second;
		for (SEM_vertex_old * n : v->neighbors) {
			if (v->id < n->id) {
...
void SEM_old::WriteParametersOfBH(string fileName) {
	...
	SEM_vertex_old * c; SEM_vertex_old * p;
	for (pair <int, SEM_vertex_old *> idPtrPair : * this->vertexMap){
		c = idPtrPair.second;
		p = c->parent;
		if (p != c) {								
...
void SEM_old::WriteRootedTreeAsEdgeList(string fileName) {
	ofstream treeFile;
	treeFile.open(fileName);
	double t;
	SEM_vertex_old * v;
...
void SEM_old::RootTreeAtAVertexPickedAtRandom() {
	...
	SEM_vertex_old * v = (*this->vertexMap)[v_ind];
	cout << "vertex selected for rooting is " << v->id << endl;
	this->RootTreeAtVertex(v);
	
}

void SEM_old::RootTreeAlongAnEdgePickedAtRandom() {
	int n = this->numberOfObservedVertices;
...
//	cout << "Number of neighbors of v are " << numOfNeighbors << endl;
	...
	SEM_vertex_old * u = v->neighbors[u_ind_in_neighborList];
//	cout << "Rooting tree along edge ";
//	cout << u->name << "\t" << v->name << endl;
	this->RootTreeAlongEdge(u,v);
}

void SEM_old::UNRESTForEachBranch() {

}

void SEM_old::RootTreeAlongEdge(SEM_vertex_old * u, SEM_vertex_old * v) {
	// Remove lengths of edges incident to root if necessary
	if (this->root->children.size() == 2) {
		SEM_vertex_old * c_l = this->root->children[0];
		SEM_vertex_old * c_r = this->root->children[1];
	...
	vector <SEM_vertex_old *> verticesToVisit;
	vector <SEM_vertex_old *> verticesVisited;
	...
	while (numberOfVerticesToVisit > 0) {		
		...
		numberOfVerticesToVisit -= 1;
		for (SEM_vertex_old * n: c->neighbors) {
...
void SEM_old::InitializeTransitionMatricesAndRootProbability() {
	// ASR via MP
	// MLE of transition matrices and root probability
		
	vector <SEM_vertex_old *> verticesToVisit;

	SEM_vertex_old * p; int numberOfPossibleStates; int pos;
	map <SEM_vertex_old *, vector <int>> VU;
	map <SEM_vertex_old *, int> V;
	for (int site = 0; site < this->num_dna_patterns; site++) {
		VU.clear(); V.clear();
		// Set VU and V for leaves;
		for (SEM_vertex_old * v : this->leaves) {
		...
		for (int v_ind = preOrderVerticesWithoutLeaves.size()-1; v_ind > -1; v_ind--) {
			...
			for (SEM_vertex_old * c : p->children) {
				for (int dna: VU[c]) {
					dnaCount[dna] += 1;
				}
			}
		...
		// Set V for ancestors
		for (SEM_vertex_old * v : this->preOrderVerticesWithoutLeaves) {
			if (v->parent == v) {
			// Set V for root
				if (VU[v].size()==1) {
					V[v] = VU[v][0];
...
void SEM_old::AddToExpectedCountsForEachVariable() {
	SEM_vertex_old * v;
	...
	vector <SEM_vertex_old *> vertexList;
...
void SEM_old::AddToExpectedCountsForEachVariablePair() {
	SEM_vertex_old * u; SEM_vertex_old * v;
	double siteWeight = this->DNAPatternWeights[this->cliqueT->site];	
	pair <SEM_vertex_old *, SEM_vertex_old*> vertexPair;
	emtr::Md countMatrixPerSite;
	for (pair<int,SEM_vertex_old*> idPtrPair_1 : *this->vertexMap) {
		u = idPtrPair_1.second;
		for (pair<int,SEM_vertex_old*> idPtrPair_2 : *this->vertexMap) {
			v = idPtrPair_2.second;			
			if (u->id < v->id) {
				if (!u->observed or !v->observed) {
					vertexPair = pair <SEM_vertex_old *, SEM_vertex_old *>(u,v);
...
void SEM_old::AddExpectedCountMatrices(map < pair <SEM_vertex_old * , SEM_vertex_old *>, emtr::Md > expectedCountsForVertexPairToAdd) {
	string u_name;
	string v_name;
	SEM_vertex_old * u;
	SEM_vertex_old * v;
	pair <SEM_vertex_old *, SEM_vertex_old *> edge;
	emtr::Md CountMatrix;
	for (pair <pair <SEM_vertex_old * , SEM_vertex_old *>, emtr::Md> mapElem: expectedCountsForVertexPairToAdd) {
		...
		if (u->id < v->id) {
			this->expectedCountsForVertexPair[pair<SEM_vertex_old*,SEM_vertex_old*>(u,v)] = CountMatrix;
		} else {			
			this->expectedCountsForVertexPair[pair<SEM_vertex_old*,SEM_vertex_old*>(v,u)] = emtr::MT(CountMatrix);
...
emtr::Md SEM_old::GetExpectedCountsForVariablePair(SEM_vertex_old * u, SEM_vertex_old * v) {
	emtr::Md C_pc;
	if (u->id < v->id) {
		C_pc = this->expectedCountsForVertexPair[pair<SEM_vertex_old*,SEM_vertex_old*>(u,v)];	
	} else {		
		C_pc = emtr::MT(this->expectedCountsForVertexPair[pair<SEM_vertex_old*,SEM_vertex_old*>(v,u)]);
	}
	return (C_pc);
}

emtr::Md SEM_old::GetPosteriorProbabilityForVariablePair(SEM_vertex_old * u, SEM_vertex_old * v) {
	emtr::Md P;
	if (u->id < v->id) {
		P = this->posteriorProbabilityForVertexPair[pair<SEM_vertex_old *, SEM_vertex_old *>(u,v)];
	} else {		
		P = emtr::MT(this->posteriorProbabilityForVertexPair[pair<SEM_vertex_old *, SEM_vertex_old *>(v,u)]);
	}
	return (P);
}

void SEM_old::AddToExpectedCountsForEachEdge() {
	double siteWeight = this->DNAPatternWeights[this->cliqueT->site];
	emtr::Md countMatrixPerSite;
	pair <SEM_vertex_old *, SEM_vertex_old *> vertexPair;
	SEM_vertex_old * u; SEM_vertex_old * v;
...
void SEM_old::AddToExpectedCounts() {
	SEM_vertex_old * u; SEM_vertex_old * v;
	double siteWeight = this->DNAPatternWeights[this->cliqueT->site];	
	// Add to counts for each unobserved vertex (C->x) where C is a clique
	array <double, 4> marginalizedProbability;
...
emtr::Md SEM_old::GetObservedCounts(SEM_vertex_old * u, SEM_vertex_old * v) {	
...
void SEM_old::ComputeExpectedCountsForTrifle(int layer) {
...
void SEM_old::ComputeExpectedCounts() {
    // cout << "Initializing expected counts" << endl;
	// cout << "11a" << endl;
	this->InitializeExpectedCountsForEachVariable();
	// cout << "11b" << endl;
...
void SEM_old::ComputeMarginalProbabilitiesUsingExpectedCounts() {	
	SEM_vertex_old * v;
	double sum;
	// Compute posterior probability for vertex
	this->posteriorProbabilityForVertex.clear();
	array <double, 4> P_X;	
	for (pair <SEM_vertex_old *, array <double, 4>> vertexAndCountArray: this->expectedCountsForVertex) {
		...
		this->posteriorProbabilityForVertex.insert(pair<SEM_vertex_old * , array <double, 4>>(v,P_X));
	}
	// Compute posterior probability for vertex pair
	this->posteriorProbabilityForVertexPair.clear();
	emtr::Md P_XY;
	pair <SEM_vertex_old *, SEM_vertex_old *> vertexPair;
	for (pair <pair<SEM_vertex_old *, SEM_vertex_old *>, emtr::Md> vertexPairAndCountMatrix: this->expectedCountsForVertexPair) {
		...
		this->posteriorProbabilityForVertexPair.insert(pair<pair<SEM_vertex_old *, SEM_vertex_old *>,emtr::Md>(vertexPair,P_XY));
	}
}

void SEM_old::ConstructCliqueTree() {
...
void SEM_old::ResetExpectedCounts() {
	SEM_vertex_old* u; SEM_vertex_old* v; 
	...
	for (pair <int, SEM_vertex_old *> idPtrPair_1 : * this->vertexMap) {
		...
		for (pair<int,SEM_vertex_old *> idPtrPair_2 : * this->vertexMap) {
			...
			if (!u->observed or !v->observed) {
				if (u->id < v->id) {
					this->expectedCountsForVertexPair[pair <SEM_vertex_old *, SEM_vertex_old *>(u,v)] = emtr::Md{};
				}	
			}			
		}
	}
...
void SEM_old::InitializeExpectedCountsForEachVariable() {
	SEM_vertex_old * v;
	...
	for (pair<int, SEM_vertex_old *> idPtrPair : * this->vertexMap) {
		...
		this->expectedCountsForVertex.insert(pair<SEM_vertex_old *, array<double,4>>(v,observedCounts));
	}	
}

void SEM_old::InitializeExpectedCountsForEachVariablePair() {
	SEM_vertex_old * u; SEM_vertex_old * v;
	...
	for (pair<int,SEM_vertex_old *> idPtrPair_1 : * this->vertexMap) {
		...
		for (pair<int,SEM_vertex_old *> idPtrPair_2 : * this->vertexMap) {
			...
			if (u->id < v->id) {
				...
				this->expectedCountsForVertexPair.insert(make_pair(pair <SEM_vertex_old *, SEM_vertex_old *>(u,v), countMatrix));
			}
		}
	}
}
...
void SEM_old::InitializeExpectedCountsForEachEdge() {
	// Initialize expected counts for each vertex pair
	SEM_vertex_old * u; SEM_vertex_old * v;
	pair <SEM_vertex_old *, SEM_vertex_old *> vertexPair;
	this->expectedCountsForVertexPair.clear();
...
void SEM_old::InitializeExpectedCounts() {
	SEM_vertex_old * u; SEM_vertex_old * v;
	// Initialize expected counts for each vertex
	this->expectedCountsForVertex.clear();
	array <double, 4> observedCounts;
...
void SEM_old::ResetPointerToRoot() {
...
void SEM_old::AddArc(SEM_vertex_old * from, SEM_vertex_old * to) {
	to->AddParent(from);
	from->AddChild(to);
}

void SEM_old::RemoveArc(SEM_vertex_old * from, SEM_vertex_old * to) {	
	to->parent = to;
	to->inDegree -= 1;
	from->outDegree -= 1;	
...
pair<bool,SEM_vertex_old *> SEM_old::CheckAndRetrieveHiddenVertexWithOutDegreeOneAndInDegreeOne() {
	bool containsVertex = 0;
	SEM_vertex_old* vPtrToReturn = (*this->vertexMap)[this->vertexMap->size()-1];
	for (pair<int, SEM_vertex_old *> idPtrPair: *this->vertexMap) {
...
pair <bool,SEM_vertex_old *> SEM_old::CheckAndRetrieveHiddenVertexWithOutDegreeOneAndInDegreeZero() {
	bool containsVertex = 0;
	SEM_vertex_old* vPtrToReturn = (*this->vertexMap)[this->vertexMap->size()-1];
	for (pair<int, SEM_vertex_old*> idPtrPair: *this->vertexMap) {
...
pair <bool, SEM_vertex_old*> SEM_old::CheckAndRetrieveSingletonHiddenVertex() {
	bool containsVertex = 0;
	SEM_vertex_old* vPtrToReturn = (*this->vertexMap)[this->vertexMap->size()-1];
	for (pair<int, SEM_vertex_old*> idPtrPair: *this->vertexMap) {
...
pair <bool,SEM_vertex_old*> SEM_old::CheckAndRetrieveHiddenVertexWithOutDegreeZeroAndInDegreeOne() {
	bool containsVertex = 0;
	SEM_vertex_old* vPtrToReturn = (*this->vertexMap)[this->vertexMap->size()-1];
	for (pair <int, SEM_vertex_old*> idPtrPair: *this->vertexMap) {
...
pair <bool, SEM_vertex_old *> SEM_old::CheckAndRetrieveHiddenVertexWithOutDegreeGreaterThanTwo() {
	bool containsVertex = 0;
	SEM_vertex_old* vPtrToReturn = (*this->vertexMap)[this->vertexMap->size()-1];
	for (pair <int, SEM_vertex_old *> idPtrPair: *this->vertexMap) {
...
pair <bool,SEM_vertex_old *> SEM_old::CheckAndRetrieveObservedVertexThatIsTheRoot() {
	bool containsVertex = 0;
	SEM_vertex_old * vPtrToReturn = (*this->vertexMap)[this->vertexMap->size()-1];
	for (pair <int, SEM_vertex_old *> idPtrPair: *this->vertexMap) {
...
pair <bool,SEM_vertex_old *> SEM_old::CheckAndRetrieveObservedVertexThatIsNotALeafAndIsNotTheRoot() {
	bool containsVertex = 0;
	SEM_vertex_old * vPtrToReturn = (*this->vertexMap)[this->vertexMap->size()-1];
	for (pair <int, SEM_vertex_old *> idPtrPair: *this->vertexMap) {
		if (idPtrPair.second->observed and idPtrPair.second->outDegree > 0) {
...
void SEM_old::RestoreRateMatricesAndScalingFactors() {
...
void SEM_old::ComputeLogLikelihoodUsingExpectedDataCompletion() {
	this->logLikelihood = 0;
	...
	for (pair <int, SEM_vertex_old *> idPtrPair : *this->vertexMap) {
		...
		if (p != c) {
			...
			if (p->id < c->id) {
				S_pc = this->expectedCountsForVertexPair[pair<SEM_vertex_old*,SEM_vertex_old*>(p,c)];	
			} else {				
				S_pc = emtr::MT(this->expectedCountsForVertexPair[pair<SEM_vertex_old*,SEM_vertex_old*>(c,p)]);
			}
			for (int dna_p = 0; dna_p < 4; dna_p ++) {
				for (int dna_c = 0; dna_c < 4; dna_c ++) {
...
void SEM_old::ComputeAALogLikelihood_maps() {
	this->AAlogLikelihood = 0;
	constexpr size_t K = 20;
	map <SEM_vertex_old*,array<double,K>> conditionalLikelihoodMap;
...
void SEM_old::ComputeAALogLikelihood() {
	int num_states = 20;
	this->AAlogLikelihood = 0;		
	...
	for (pair<SEM_vertex_old *,SEM_vertex_old *> edge : this->edgesForPostOrderTreeTraversal) {
		tie (p, c) = edge;
...
void SEM_old::ComputeTrifleLogLikelihood(int layer) {
	this->logLikelihood = 0;
	map <SEM_vertex_old*,array<double,4>> conditionalLikelihoodMap;
...
std::string SEM_old::emtrifle_to_json(const EMTrifle_struct& em) const {
    std::ostringstream os;
...
// Case 2: Root may have out degree = one
// Case 3: Directed tree (rooted) with vertices with outdegree 2 or 0.
void SEM_old::ComputeLogLikelihood() {
	this->logLikelihood = 0;
	map <SEM_vertex_old*,array<double,4>> conditionalLikelihoodMap;
	std::array <double,4> conditionalLikelihood;
	double partialLikelihood;
...
vector<int> SEM_old::DecompressSequence(vector<int> compressedSequence, vector<vector<int>> sitePatternRepeats){
	int totalSequenceLength = 0;
	for (vector<int> sitePatternRepeat: sitePatternRepeats){
		totalSequenceLength += int(sitePatternRepeat.size());
	}
...
string SEM_old::EncodeAsDNA(vector<int> sequence){
	string allDNA = "AGTC";
	string dnaSequence = "";
	for (int s : sequence){
		dnaSequence += allDNA[s];
...
void SEM_old::Optimize_AA_RateMatrix_rooted_at_each_internal_vertex(int num_repetitions) {
	int n = this->numberOfObservedVertices;
...
void SEM_old::EM_DNA_rooted_at_each_internal_vertex_started_with_dirichlet_store_results(int num_repetitions) {
	int n = this->numberOfObservedVertices;
	int num_vertices = this->vertexMap->size();	
...
void SEM_old::EMtrifle_DNA_rooted_at_each_internal_vertex_started_with_dirichlet_store_results() {
  int n = this->numberOfObservedVertices;
  int num_vertices = this->vertexMap->size();
...
void SEM_old::EM_rooted_at_each_internal_vertex_started_with_dirichlet(int num_repetitions) {
	int n = this->numberOfObservedVertices;
	int num_vertices = this->vertexMap->size();	
	SEM_vertex_old * v;
...
void SEM_old::EMTrifle_DNA_for_replicate(int rep) {
...
void SEM_old::EMTrifle_DNA_rooted_at_each_internal_vertex_started_with_parsimony_store_results() {
  int n = this->numberOfObservedVertices;
  int num_vertices = this->vertexMap->size();
...
void SEM_old::EM_DNA_rooted_at_each_internal_vertex_started_with_parsimony_store_results(int num_repetitions) {
	int n = this->numberOfObservedVertices;	
	int num_vertices = this->vertexMap->size();	
...
void SEM_old::EM_rooted_at_each_internal_vertex_started_with_parsimony(int num_repetitions) {
	int n = this->numberOfObservedVertices;
	int num_vertices = this->vertexMap->size();	
	SEM_vertex_old * v;
	vector <double> loglikelihoodscoresForEachRepetition;
...
void SEM_old::EM_DNA_rooted_at_each_internal_vertex_started_with_HSS_store_results(int num_repetitions) {
	int n = this->numberOfObservedVertices;
	int num_vertices = this->vertexMap->size();	
...
void SEM_old::EMtrifle_DNA_rooted_at_each_internal_vertex_started_with_HSS_store_results() {
  int n = this->numberOfObservedVertices;
  int num_vertices = this->vertexMap->size();
...
void SEM_old::EM_rooted_at_each_internal_vertex_started_with_HSS_par(int num_repetitions) {
...
void SEM_old::EMTrifle_started_with_dirichlet_rooted_at(SEM_vertex_old *v, int layer) {
...
void SEM_old::EMTrifle_started_with_HSS_rooted_at(SEM_vertex_old *v, int layer) {	
	// iterate over each internal node	
	this->RootTreeAtVertex(v);
	if (layer == 0) {
...
void SEM_old::EMTrifle_started_with_parsimony_rooted_at(SEM_vertex_old *v, int layer) {
	// iterate over each internal node	
	this->RootTreeAtVertex(v);
	if (layer == 0) {		
...
tuple <int,double,double,double,double> SEM_old::EM_started_with_parsimony_rooted_at(SEM_vertex_old *v) {
	//	cout << "10a" << endl;
	// iterate over each internal node	
	this->RootTreeAtVertex(v);
	// cout << "10b" << endl;
...
tuple <int,double,double,double,double> SEM_old::EM_started_with_HSS_parameters_rooted_at(SEM_vertex_old *v) {
	//	cout << "10a" << endl;
	// iterate over each internal node	
	this->RootTreeAtVertex(v);
	// cout << "10b" << endl;	
...
tuple <int,double,double,double,double> SEM_old::EM_started_with_dirichlet_rooted_at(SEM_vertex_old *v) {
	//	cout << "10a" << endl;
	// iterate over each internal node	
...
void SEM_old::ComputeSumOfExpectedLogLikelihoods() {
...
void SEM_old::ComputeMLRootedTreeForRootSearchUnderBH() {
	vector < SEM_vertex_old *> verticesToVisit = this->preOrderVerticesWithoutLeaves;	
	double logLikelihood_max = 0;
	int numberOfVerticesVisited = 0;	
	for (SEM_vertex_old * v : verticesToVisit) {
...
void SEM_old::ComputeMLRootedTreeForFullStructureSearch() {
...
emtr::Md SEM_old::GetP_yGivenx(emtr::Md P_xy) {
...
void SEM_old::SetMinLengthOfEdges() {	
	for (pair<pair<SEM_vertex_old * , SEM_vertex_old * >,double> edgeAndLengthsPair: this->edgeLengths){		
...
void SEM_old::ComputeMLEstimateOfBHGivenExpectedDataCompletion() {	
	SEM_vertex_old * x; SEM_vertex_old * y;
	emtr::Md P_xy;
	for (pair <int, SEM_vertex_old*> idPtrPair : *this->vertexMap) {
		y = idPtrPair.second;
		x = y->parent;
...
void SEM_old::StoreEdgeListForChowLiuTree() {
...
void SEM_old::RestoreEdgeListForChowLiuTree() {
	this->ClearDirectedEdges();
	SEM_vertex_old *u; SEM_vertex_old *v; 
	for(pair<SEM_vertex_old *,SEM_vertex_old*> edge : this->edgesForChowLiuTree){
		tie (u, v) = edge;
		u->AddNeighbor(v);
		v->AddNeighbor(u);
	}
...
void SEM_old::StoreDirectedEdgeList() {
	SEM_vertex_old * p;
	SEM_vertex_old * c;
...
void SEM_old::RestoreDirectedEdgeList() {
	this->ClearDirectedEdges();
	SEM_vertex_old * p;
	SEM_vertex_old * c;
	for (pair <SEM_vertex_old *, SEM_vertex_old *> edge : this->directedEdgeList) {
		tie (p, c) = edge;
		c->AddParent(p);
		p->AddChild(c);		
	}
...
void SEM_old::RootTreeAtVertex(SEM_vertex_old* r) {	
	this->ClearDirectedEdges();
	vector <SEM_vertex_old*> verticesToVisit;
	vector <SEM_vertex_old*> verticesVisited;
	SEM_vertex_old * p;	
	...
	while (numberOfVerticesToVisit > 0) {
		...
		numberOfVerticesToVisit -= 1;
		for (SEM_vertex_old* c : p->neighbors) {
...
void SEM_old::TransformRootedTreeToBifurcatingTree() {
	bool containsMatchingVertex;
	bool containsSingletonHiddenVertex;
	bool debug = 0;
	SEM_vertex_old * matchingVertex;
	SEM_vertex_old * p; SEM_vertex_old * c; SEM_vertex_old * h;
	SEM_vertex_old * o; SEM_vertex_old * h_s;
	SEM_vertex_old * c_1; SEM_vertex_old * c_2; 
	emtr::Md P;
	array <double, 4> rootProb_orig;
	array <double, 4> rootProb_new;
	vector <SEM_vertex_old *> childrenToSwap;
...
//			cout << "Edge (p,c) added" << endl;			
		...
		if (this->root->observed) {
			...
			for (SEM_vertex_old * child : childrenToSwap) {
...
//			cout << o->name  << endl;
			...
			for (SEM_vertex_old * c: childrenToSwap){					
				this->RemoveArc(o,c);
				this->AddArc(h_s,c);
			}
		...
		while (containsMatchingVertex and containsSingletonHiddenVertex) {			
			...
			sort(childrenToSwap.begin(),childrenToSwap.end(), [](SEM_vertex_old * u, SEM_vertex_old * v) {
				return u->id < v->id;
			});
...
void SEM_old::ClearDirectedEdges() {
...
void SEM_old::ClearUndirectedEdges() {
	SEM_vertex_old * v;
	for (pair <int, SEM_vertex_old *> idPtrPair : * this->vertexMap) {
		v = idPtrPair.second;
		v->degree = 0;
...
void SEM_old::ResetTimesVisited() {
	SEM_vertex_old * v;
	for (pair <int, SEM_vertex_old *> idPtrPair : * this->vertexMap) {
		v = idPtrPair.second;		
		v->timesVisited = 0;		
...
array <double, 4> SEM_old::GetBaseCompositionForTrifle(SEM_vertex_old * v, int layer) {
...
array <double, 4> SEM_old::GetBaseComposition(SEM_vertex_old * v) {
	array <double, 4> baseCompositionArray;
	for (int dna = 0; dna < 4; dna ++) {
		baseCompositionArray[dna] = 0;
	}
...
array <double, 4> SEM_old::GetObservedCountsForVariable(SEM_vertex_old * v) {
	array <double, 4> observedCounts;
	for (int i = 0; i < 4; i++) {
		observedCounts[i] = 0;
 	}
...
void SEM_old::ComputeMLEOfRootProbability() {
	this->rootProbability = GetBaseComposition(this->root);
	this->root->rootProbability = this->rootProbability;
}

void SEM_old::ComputeMLEOfTransitionMatrices() {
	SEM_vertex_old * c; SEM_vertex_old * p;
	bool debug = 0;
	for (pair <int, SEM_vertex_old *> idPtrPair : * this->vertexMap){
		c = idPtrPair.second;
...
void SEM_old::SetInitialEstimateOfModelParametersUsingDirichlet() {

	array <double, 4> pi_diri = SEM_old::sample_pi();

	this->rootProbability = pi_diri;
	this->root->rootProbability = pi_diri;

	SEM_vertex_old * p; SEM_vertex_old * c; emtr::Md M_pc;
	for (pair<SEM_vertex_old*,SEM_vertex_old*> edge : this->edgesForPostOrderTreeTraversal) {		
		p = edge.first;
		c = edge.second;

		for (int row = 0; row < 4; row++) {
			array <double, 4> M_row_diri = SEM_old::sample_M_row();
			int diri_index = 1;
			for (int col = 0; col < 4; col++) {
				if (row == col){
...
void SEM_old::StoreInitialParamsInEMTrifleCurrent(int layer) {	
	SEM_vertex_old * c;		
...
void SEM_old::StoreFinalParamsInEMTrifleCurrent(int layer) {	
	SEM_vertex_old * c;		
...
void SEM_old::StoreParamsInEMCurrent(string init_or_final) {
	for (int dna = 0; dna < 4; dna ++) {
...
void SEM_old::SetInitialEstimateOfModelParametersUsingHSS() {
...
void SEM_old::BumpZeroEntriesOfModelParameters() {
...
void SEM_old::ComputeInitialEstimateOfModelParametersForTrifle(int layer) {
	bool debug = 0;
	// cout << "here 11a" << endl;
...
void SEM_old::ComputeInitialEstimateOfModelParameters() {
	bool debug = 0;
	this->rootProbability = GetBaseComposition(this->root);	
	this->root->rootProbability = this->rootProbability;
	if (debug) {
...
void SEM_old::ResetLogScalingFactors() {
	for (pair <int, SEM_vertex_old * > idPtrPair : * this->vertexMap){
		idPtrPair.second->logScalingFactors = 0;
	}
}

void SEM_old::ComputeMPEstimateOfAncestralSequencesForTrifle(int layer) {
	SEM_vertex_old * p;	
	map <SEM_vertex_old * , int> V;
	map <SEM_vertex_old * , vector<int>> VU;					
	map <int, int> dnaCount;
	...
	//	Compute V and VU for leaves	
		...
		for (SEM_vertex_old * c : this->leaves) {			
	...
	//	Set VU for ancestors
		for (SEM_vertex_old * c : this->preOrderVerticesWithoutLeaves) {
			vector <int> vectorToAdd;
			VU.insert(make_pair(c,vectorToAdd));
		}				
	...
	// Set V for ancestors
		for (SEM_vertex_old * c : preOrderVerticesWithoutLeaves) {
			if (c->parent == c) {			
			// Set V for root
				if (VU[c].size()==1) {
...
void SEM_old::ComputeMPEstimateOfAncestralSequences() {
	SEM_vertex_old * p;	
	map <SEM_vertex_old * , int> V;
	map <SEM_vertex_old * , vector<int>> VU;					
	map <int, int> dnaCount;
...
void SEM_old::ComputeMAPEstimateOfAncestralSequences() {
	...
	for (int site = 0 ; site < this->num_dna_patterns; site++){
		...
		for (pair<SEM_vertex_old *,SEM_vertex_old *> edge : this->edgesForPostOrderTreeTraversal){
			...
			if (c->observed) {
				...
				conditionalLikelihoodMap.insert(pair<SEM_vertex_old *,array<double,4>>(c,conditionalLikelihood));
			}
			...
			if (conditionalLikelihoodMap.find(p) == conditionalLikelihoodMap.end()){
				...
				conditionalLikelihoodMap.insert(pair<SEM_vertex_old *,array<double,4>>(p,conditionalLikelihood));
			}
			largestConditionalLikelihood = 0;
			for (int dna_p = 0; dna_p < 4; dna_p++) {
				partialLikelihood = 0;
...
void SEM_old::ComputePosteriorProbabilitiesUsingMAPEstimates() {
	this->posteriorProbabilityForVertexPair.clear();
	emtr::Md P;
	SEM_vertex_old * u;
	SEM_vertex_old * v;
...
double SEM_old::GetExpectedMutualInformation(SEM_vertex_old * x, SEM_vertex_old* y) {
	pair <SEM_vertex_old *, SEM_vertex_old *> vertexPair;
	if (x->id < y->id) {
		vertexPair = pair <SEM_vertex_old *, SEM_vertex_old *>(x,y);
	} else {
		vertexPair = pair <SEM_vertex_old *, SEM_vertex_old *>(y,x);
	}
...
void SEM_old::ComputeChowLiuTree() {
	this->ClearAllEdges();
...
void SEM_old::SetEdgesForTreeTraversalOperations() {
	this->SetEdgesForPreOrderTraversal();
	this->SetEdgesForPostOrderTraversal();
	this->SetVerticesForPreOrderTraversalWithoutLeaves();	
	this->SetLeaves();
...
void SEM_old::SetEdgesForPreOrderTraversal() {
	this->edgesForPreOrderTreeTraversal.clear();
	vector <SEM_vertex_old*> verticesToVisit;	
	SEM_vertex_old * p;	
	...
	while (numberOfVerticesToVisit > 0) {
		...
		numberOfVerticesToVisit -= 1;
		for (SEM_vertex_old* c : p->children){
			this->edgesForPreOrderTreeTraversal.push_back(pair<SEM_vertex_old*, SEM_vertex_old*>(p,c));
			verticesToVisit.push_back(c);
			numberOfVerticesToVisit += 1;
		}
	}
...
void SEM_old::SetEdgesForPostOrderTraversal() {	
	vector <SEM_vertex_old*> verticesToVisit;	
	SEM_vertex_old* c;
	SEM_vertex_old* p;	
	for (pair<int,SEM_vertex_old*> idPtrPair : *this->vertexMap){
	...
	while (numberOfVerticesToVisit > 0) {
		...
		if (c != c->parent) {
			p = c->parent;
			this->edgesForPostOrderTreeTraversal.push_back(pair<SEM_vertex_old*, SEM_vertex_old*>(p,c));
			p->timesVisited += 1;
...
void SEM_old::SetVerticesForPostOrderTraversal() {

}

void SEM_old::SetVerticesForPreOrderTraversalWithoutLeaves() {
...
void SEM_old::RootedTreeAlongAnEdgeIncidentToCentralVertex() {	
...
bool SEM_old::IsNumberOfNonSingletonComponentsGreaterThanZero() {	
	bool valueToReturn;
...
void SEM_old::SelectIndsOfVerticesOfInterestAndEdgesOfInterest() {
	this->SuppressRoot();
	vector <SEM_vertex_old *> verticesToVisit;
	vector <SEM_vertex_old *> verticesVisited;
	int n_ind; SEM_vertex_old * v;
	...
	for (SEM_vertex_old * v: verticesVisited) {	
		if (!v->observed) {
...
void SEM_old::RenameHiddenVerticesInEdgesOfInterestAndSetIdsOfVerticesOfInterest() {		
	vector <SEM_vertex_old *> vertices;	
...
void SEM_old::SetIdsOfExternalVertices() {	
	this->idsOfExternalVertices.clear();
	SEM_vertex_old * v;
...
void SEM_old::SetInfoForVerticesToAddToMST(){
...
void SEM_old::AddSitePatternWeights(vector <int> sitePatternWeightsToAdd) {
...
void SEM_old::AddSitePatternRepeats(vector <vector <int> > sitePatternRepetitionsToAdd) {
	this->sitePatternRepetitions = sitePatternRepetitionsToAdd;
}

void SEM_old::SetNumberOfVerticesInSubtree(int numberOfVerticesToSet) {
	this->numberOfVerticesInSubtree = numberOfVerticesToSet;
}

void SEM_old::AddNames(vector <string> namesToAdd) {
...
void SEM_old::AddGlobalIds(vector <int> idsToAdd) {
	if (this->numberOfObservedVertices == 0) {
...
void SEM_old::AddSequences(vector <vector <int>> sequencesToAdd) {
	this->numberOfObservedVertices = sequencesToAdd.size();
	this->node_ind = this->numberOfObservedVertices;
	for (int i = 0 ; i < this->numberOfObservedVertices; i++) {
		SEM_vertex_old * v = new SEM_vertex_old(i,sequencesToAdd[i]);
		v->observed = 1;
		this->vertexMap->insert(make_pair(i,v));
	}	
}

void SEM_old::AddRootVertex() {
	int n = this->numberOfObservedVertices;
	vector <int> emptySequence;	
	this->root = new SEM_vertex_old (-1,emptySequence);
...
void SEM_old::SetEdgesFromTopologyFile() {
	SEM_vertex_old * u; SEM_vertex_old * v;
	vector <string> splitLine;
	...
	for(string line; getline(inputFile, line );) {
		...
		if (this->ContainsVertex(u_name)) {
			...
			u = new SEM_vertex_old(this->node_ind,emptySequence);
			u->name = u_name;
			u->id = this->node_ind;
			this->vertexMap->insert(pair<int,SEM_vertex_old*>(u->id,u));
		...
		if (this->ContainsVertex(v_name)) {
			...
			v = new SEM_vertex_old(this->node_ind,emptySequence);
			v->name = v_name;
			v->id = this->node_ind;
			this->vertexMap->insert(pair<int,SEM_vertex_old*>(v->id,v));
...
void SEM_old::CompressAASequences() {			
	...
	for (unsigned int i = 0; i < this->leaves.size(); i++) {
		SEM_vertex_old * l_ptr = this->leaves[i];
	...
	for(int site = 0; site < numberOfSites; site++) {
		sitePattern.clear();
		for (SEM_vertex_old* l_ptr: this->leaves) {
...
void SEM_old::CompressDNASequences() {
    this->DNAPatternWeights.clear();
    ...
    for (size_t k = 0; k < order.size(); ++k) {
        ...
        // reorder all pattern-level metadata
...
void SEM_old::CompressDNASequencesSep20() {
...
void SEM_old::CompressDNASequencesBkp() {			
	this->DNAPatternWeights.clear();	
	vector <vector<int>> uniquePatterns;
	this->gapLessDNAFlag.clear();
	map <vector<int>,vector<int>> uniquePatternsToSitesWherePatternRepeats;	
...
void SEM_old::SetDNASequencesFromFile(string sequenceFileName) {
	this->leaves.clear();
...
void SEM_old::SetAASequencesFromFile(string sequenceFileName) {
...
void SEM_old::AddWeightedEdges(vector <tuple <string,string,double> > weightedEdgesToAdd) {
	SEM_vertex_old * u; SEM_vertex_old * v;
	string u_name; string v_name; double t;
	vector <int> emptySequence;
	int edge_count_h1 = 0;
...
void SEM_old::AddEdgeLogLikelihoods(vector<tuple<string,string,double>> edgeLogLikelihoodsToAdd) {
	SEM_vertex_old * u; SEM_vertex_old * v; double edgeLogLikelihood;
	string u_name; string v_name;
	pair<SEM_vertex_old *, SEM_vertex_old *> vertexPair;
	for (tuple<string,string,double> edgeLogLikelihoodTuple : edgeLogLikelihoodsToAdd) {
		...
		v = (*this->vertexMap)[this->nameToIdMap[v_name]];
		vertexPair = pair <SEM_vertex_old *, SEM_vertex_old *> (u,v);
		this->edgeLogLikelihoodsMap.insert(pair<pair <SEM_vertex_old *, SEM_vertex_old *>,double>(vertexPair, edgeLogLikelihood));
	}	
}

void SEM_old::AddVertexLogLikelihoods(map<string,double> vertexLogLikelihoodsMapToAdd) {
	string v_name; SEM_vertex_old * v; double vertexLogLikelihood;
...
bool SEM_old::ContainsVertex(string v_name) {	
...
void SEM_old::SetAASequenceForVertex(string v_name, vector<int> aaRecoded){
	SEM_vertex_old * v = this->GetVertex(v_name);
	v->AArecoded = aaRecoded;
}

void SEM_old::AddVertex(string u_name, vector <int> emptySequence) {	
	SEM_vertex_old * u = new SEM_vertex_old(this->node_ind,emptySequence);
	u->name = u_name;
	u->id = this->node_ind;
	this->node_ind++;
	this->vertexMap->insert(pair<int,SEM_vertex_old*>(u->id,u));
...
double SEM_old::ComputeDistance(int v_i, int v_j) {
...
EMManager::EMManager(string DNAsequenceFileNameToSet,
		...
		this->P = new SEM_old(0.0,100,this->verbose);
