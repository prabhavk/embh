// patterns_cuda_newick.cu
// Build unique site-patterns from a FASTA alignment using CUDA,
// with taxon ordering determined by post-order traversal of a Newick tree.
//
// Outputs:
...
//   <prefix>.taxon_order  (CSV: taxon_name,position_in_order)
//
// Compile: nvcc -O3 -std=c++11 patterns_cuda_newick.cu -o patterns_cuda_newick
// Usage:   ./patterns_cuda_newick input.fasta input.newick [output_prefix]
...
private:
  ...
  void skip_whitespace() {
    while (text[pos] && std::isspace(text[pos])) pos++;
  }
  
  std::unique_ptr<TreeNode> parse_node() {
    skip_whitespace();
    ...
    if (text[pos] == '(') {
      ...
      while (true) {
        ...
        if (text[pos] == ',') {
          ...
          die("Expected ',' or ')' in Newick tree");
        }
      }
    }
    
    // Parse label (taxon name or internal node label)
    skip_whitespace();
    std::string label;
    while (text[pos] && text[pos] != ':' && text[pos] != ',' && 
           text[pos] != ')' && text[pos] != ';' && !std::isspace(text[pos])) {
      label += text[pos++];
    }
    node->name = label;
    ...
    if (text[pos] == ':') {
      ...
      skip_whitespace();
      char* end;
      node->branch_length = strtod(text + pos, &end);
      pos = end - text;
    }
...
public:
  std::unique_ptr<TreeNode> parse(const std::string& newick_str) {
    text = newick_str.c_str();
    pos = 0;
    skip_whitespace();
    
    std::unique_ptr<TreeNode> root = parse_node();
...
// Post-order traversal to collect leaf taxa in order
void post_order_traversal(const TreeNode* node, std::vector<std::string>& taxon_order) {
  if (!node) return;
  
  // Visit children first (post-order)
  for (const auto& child : node->children) {
    post_order_traversal(child.get(), taxon_order);
  }
  
  // Then visit this node - add to order if it's a leaf (has a name and no children)
  if (node->children.empty() && !node->name.empty()) {
    taxon_order.push_back(node->name);
  }
}

std::vector<std::string> get_taxon_order_from_newick(const std::string& newick_str) {
  NewickParser parser;
  std::unique_ptr<TreeNode> tree = parser.parse(newick_str);
  
  std::vector<std::string> taxon_order;
  post_order_traversal(tree.get(), taxon_order);
...
int main(int argc, char** argv) {
  if (argc < 3) {
    fprintf(stderr, "Usage: %s <input.fasta> <input.newick> [output_prefix]\n", argv[0]);
    return 1;
  }
  const char* fasta_path = argv[1];
  const char* newick_path = argv[2];
  const char* out_prefix_arg = (argc>=4? argv[3] : nullptr);

  // Read Newick tree and get taxon order from post-order traversal
  std::string newick_str = read_file_to_string(newick_path);
  std::vector<std::string> taxon_order = get_taxon_order_from_newick(newick_str);
  
  fprintf(stderr, "Post-order traversal yielded %zu taxa\n", taxon_order.size());

  // Read FASTA
  SeqVec sv_original;
  read_fasta(fasta_path, sv_original);
  
  fprintf(stderr, "Read %zu sequences from FASTA\n", sv_original.v.size());
  
  // Create a map from taxon name to sequence
  std::unordered_map<std::string, Seq> seq_map;
  for (const auto& s : sv_original.v) {
    seq_map[s.name] = s;
  }
  
  // Reorder sequences according to taxon_order
  SeqVec sv;
  for (const auto& taxon_name : taxon_order) {
    auto it = seq_map.find(taxon_name);
    if (it == seq_map.end()) {
      fprintf(stderr, "Warning: Taxon '%s' from tree not found in FASTA\n", taxon_name.c_str());
      continue;
    }
    sv.v.push_back(it->second);
  }
  
  if (sv.v.empty()) die("No matching sequences found between tree and FASTA");
  
  const size_t nseq = sv.v.size();
  const size_t L    = sv.v[0].seq.size();
  
  fprintf(stderr, "Using %zu sequences in tree-based order\n", nseq);
  ...
  char path_pat[1200], path_bc[1200], path_taxon_order[1200];
  snprintf(path_pat, sizeof(path_pat), "%s.pat",      prefix);
  snprintf(path_bc,  sizeof(path_bc),  "%s.basecomp", prefix);
  snprintf(path_taxon_order, sizeof(path_taxon_order), "%s.taxon_order", prefix);

  FILE* fpat = fopen(path_pat, "w");   if (!fpat) die("cannot write .pat");
  FILE* fbc  = fopen(path_bc,  "w");   if (!fbc)  die("cannot write .basecomp");
  FILE* ftax = fopen(path_taxon_order, "w"); if (!ftax) die("cannot write .taxon_order");
  ...
  // Taxon order file (CSV format)
  fprintf(ftax, "taxon_name,position\n");
  for (size_t i = 0; i < taxon_order.size(); ++i) {
    fprintf(ftax, "%s,%zu\n", taxon_order[i].c_str(), i);
  }

  fclose(fpat); fclose(fbc); fclose(ftax);

  fprintf(stderr, "Wrote:\n");
  fprintf(stderr, "  %s  (weight + pattern in tree order)\n", path_pat);
  fprintf(stderr, "  %s  (base composition + gap stats)\n", path_bc);
  fprintf(stderr, "  %s  (taxon order from post-order traversal)\n", path_taxon_order);
